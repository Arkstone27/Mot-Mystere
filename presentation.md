...
5. Problème les notes sont énormes (cf figure 1) et mal répartit pour cela. Pour ce faire il faut appliquer une fonction de linéarisation (ex: divisé par le maximum), ici j'ai utilisé la fonction sigmoide. Mais il faut préparé les notes avant pour cela on commence par faire le Z-Score des notes. On soustrais chaque note par la moyenne des notes puis on divise par l'écart type des notes. Ensuite on multiplie par 10 et on obtient ainsi des notes de 0 à 10 pour chaque mot (10 étant le mot le plus "complexe" à trouvé).
